...
bool Competicao::adicionar(Modalidade* m) {
    ...
    for(int i = 0; i < quantidadeDeModalidades; i++) {
        ...
        Equipe** equipesDaModalidade = m->getEquipes();

        bool equipeJaCadastrada;

        for(int j = 0; j < m->getQuantidade(); j++){
            equipeJaCadastrada = false;
            for(int z = 0; z < this->getQuantidadeDeEquipes(); z++){
                if(equipesDaModalidade[j] == equipes[z]){
                    equipeJaCadastrada = true;
                }
            }
...
//
//           if(this->adicionar(equipesDaModalidade[i]) == true){
//              return false;
//           }
//
//    }
